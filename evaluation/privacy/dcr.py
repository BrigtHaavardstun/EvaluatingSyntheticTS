...
def evaluate_privacy(synthetic_train_x, real_train_x):
    ...
    for synth_x in tqdm(selected_synth_data):
        minimum_dist = find_dcr(synth_x,real_train_x)
        all_distances.append(minimum_dist)

    min_overall = np.min(all_distances)
    avg_drc = np.mean(all_distances)
    return min_overall, avg_drc
...
def main():
    ...
    for dataset_name in dataset_names:
        ...
        results_dir = f"Results/PRIVACY/DCR/{dataset_name}"
