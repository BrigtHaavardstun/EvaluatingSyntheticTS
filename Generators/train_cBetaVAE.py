...
def define_model(T, y):
    seq_len = T
    feat_dim = 1
    latent_dim = max(4, min(T//4,32)) # At least 4, but for large time series, max 32
    ...
    cBetaVae = tsgm.models.cvae.cBetaVAE(encoder=encoder, decoder=decoder, latent_dim=latent_dim, beta=2,
                                         temporal=False)
...
class cBetaVAE(BaseGenerator):
    ...
    def sample(self, T, num_classes, nr_samples):
        ...
        gen_samples, gen_label = self.model.generate(tensor)
        ...
        return gen_data, self.label_encoder.inverse_transform(gen_label).flatten()
    ...
    def train(self,dataset_name = "Chinatown", epochs=1000, nr_samples=10)->None:
        ...
        self.model.fit(x=ts_data, y=labels, epochs=epochs, batch_size=min(8, T//6),verbose=False)
...
def run():
    ...
    load_and_visualize(dataset_name=dataset_name, generator_name=gan.get_name(), epochs=epochs)
